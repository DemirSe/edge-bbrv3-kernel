...
for sym in WIRELESS WLAN CFG80211 MAC80211 BT SOUND SND DRM DRM_BOCHS USB USB_SUPPORT FLOPPY ATA ATA_PIIX ATA_GENERIC CDROM BLK_DEV_SR ISO9660_FS UDF_FS VMWARE_VMCI VSOCKETS VIRTIO_VSOCKETS TUN IPV6_SIT IPV6_GRE IPV6_TUNNEL IPV6_VTI DEBUG_INFO DEBUG_KERNEL GDB_SCRIPTS KGDB; do
  scripts/config --disable "$sym" || true
done

# Disable all legacy xtables / iptables / ebtables / arptables symbols that survived Debian localmodconfig.
for sym in $(grep -E '^CONFIG_(NFT_COMPAT|NETFILTER_XTABLES|NETFILTER_XT_|IP_NF_|IP6_NF_|IPT_|IP6T_|EBT_|BRIDGE_EBT|ARP_TABLES|ARPT_)' .config | sed -E 's/^CONFIG_([^=]+)=.*/\1/'); do
  scripts/config --disable "$sym" || true
done
scripts/config --disable NETFILTER_XTABLES || true
scripts/config --disable NETFILTER_XTABLES_COMPAT || true
scripts/config --disable NFT_COMPAT || true
